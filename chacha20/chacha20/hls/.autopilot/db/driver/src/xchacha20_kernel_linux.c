...
typedef struct {
    ...
    XChacha20_kernel_uio_map maps[ MAX_UIO_MAPS ];
} XChacha20_kernel_uio_info;

/***************** Variable Definitions **************************************/
static XChacha20_kernel_uio_info uio_info;
...
static int uio_info_read_name(XChacha20_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XChacha20_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XChacha20_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XChacha20_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XChacha20_kernel_Initialize(XChacha20_kernel *InstancePtr, const char* InstanceName) {
	XChacha20_kernel_uio_info *InfoPtr = &uio_info;
